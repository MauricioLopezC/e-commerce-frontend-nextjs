...
export async function getAllDiscounts(
  ...
  const res = await fetch(
    `${BACKEND_URL}/promotions/discounts?${queryParams.toString()}`,
    {
      method: 'GET',
      next: {
        tags: ['discounts'],
...
export async function connectProducts(
  ...
  const token = cookies().get('access-token')?.value;
  const res = await fetch(
    `${BACKEND_URL}/promotions/discounts/${discountId}/products`,
    {
      headers: {
        'Content-Type': 'application/json',
        Cookie: `access-token=${token}`,
...
export async function connectCategories(
  ...
  const token = cookies().get('access-token')?.value;
  const res = await fetch(
    `${BACKEND_URL}/promotions/discounts/${discountId}/categories`,
    {
      headers: {
        'Content-Type': 'application/json',
        Cookie: `access-token=${token}`,
...
export async function disconnectCategories(
  discountId: number,
  categoryIds: number[],
): Promise<OneDiscountResponse> {
  const token = cookies().get('access-token')?.value;
